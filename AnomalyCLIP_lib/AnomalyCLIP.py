...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        ...
        # Interpolate functionally: mutating this parameter here makes a
        # high-resolution forward followed by a low-resolution TPT forward
        # depend on call order and permanently changes the loaded checkpoint.
        pos = self.positional_embedding
        if side != new_side:
            new_pos = pos[1:, :].reshape(-1, side, side, x.shape[-1]).permute(0, 3, 1, 2)
            ...
            pos = torch.cat([pos[:1, :], new_pos[0]], 0)
